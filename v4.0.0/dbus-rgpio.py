            ...
            if topic.startswith(API_DEVICE_REGISTER_TOPIC):
                ...
                try:
                    if len(parts) == 7: # rgpio/api/device/relay/read/SERIAL/NUMBER
                        serial = parts[5]
                        relay_index = int(parts[6]) - 1 # convert to 0-based
                        serial_safe = serial.replace('-', '_')
                        if serial_safe in self.relay_services:
                            self.relay_services[serial_safe].update_state_from_api(relay_index, payload)
                    else:
                        logger.warning(f"Malformed relay read topic: {topic}")
                except (IndexError, ValueError) as e:
                    logger.warning(f"Could not parse relay read topic: {topic}. Error: {e}")
